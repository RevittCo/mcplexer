...
Three commands. Clone, install, done.
...
`make install` builds MCPlexer, starts the daemon, configures Claude Desktop, and opens the dashboard. Restart Claude Desktop and you're live.
...
`make install` ran two things:

1. **`make build`** — compiled the Go binary and built the web UI
2. **`mcplexer setup`** — started the daemon, wrote the Claude Desktop config, and opened the dashboard

Your Claude Desktop config now has an `mx` entry pointing at MCPlexer's Unix socket. Every MCP tool call from Claude goes through MCPlexer first.
