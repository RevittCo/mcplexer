...
User=mcplexer
Group=mcplexer
ExecStart=/usr/local/bin/mcplexer serve --mode=http --addr=:8080
Restart=on-failure
RestartSec=5
...
| `MCPLEXER_HTTP_ADDR` | HTTP listen address | `:8080` |
...
MCPlexer encrypts secrets at rest using [age](https://age-encryption.org/) keys. **Back up your age identity file** — if lost, all encrypted secrets become unrecoverable.
