...
- **Codex Dynamic Tool Compat** — automatically include dynamic-discovery tools in `tools/list` for Codex sessions, since Codex doesn't support the `load_tools` pattern (default: on)

### Caching

- **Tools List Cache TTL** — how long the aggregated `tools/list` response is cached before re-querying downstream servers (0–300 seconds, default: 15)
...
<Callout type="info" title="Environment variable overrides">
