...
<ApiEndpoint method="GET" path="/api/v1/settings" description="Get current settings. Returns the active settings (merged from defaults, database, and environment variable overrides) plus the default descriptions for all built-in tools." />

<ApiEndpoint method="PUT" path="/api/v1/settings" description="Update settings. Validates and persists to the database. Environment variable overrides still take precedence on read.">
...
<ApiEndpoint method="GET" path="/api/v1/mcp-install/status" description="List all known MCP clients with detection status (installed on this machine), configuration status (MCPlexer entry present), binary path, and the server entry JSON." />

<ApiEndpoint method="POST" path="/api/v1/mcp-install/{clientId}/install" description="Write the MCPlexer server entry into a client's MCP config file. The client must be detected on this machine." />

<ApiEndpoint method="POST" path="/api/v1/mcp-install/{clientId}/uninstall" description="Remove the MCPlexer server entry from a client's MCP config file." />

<ApiEndpoint method="GET" path="/api/v1/mcp-install/{clientId}/preview" description="Preview what the client's config file would look like after installing MCPlexer, without writing any changes." />
